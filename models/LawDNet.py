...
class LawDNet(nn.Module):
    ...
    def forward(self, source_img,ref_img,audio_feature):
        with autocast():
            ...
            # img_para = self.global_avg2d(img_para).squeeze(3).squeeze(2) # 5*batchsize 128 origin

            ## audio encoder
            audio_para = self.audio_encoder(audio_feature) # 5*batchsize 128 2
            # audio_para = self.global_avg1d(audio_para).squeeze(2) # 5*batchsize 128 origin

            ## concat alignment feature and audio feature
            # trans_para = torch.cat([img_para,audio_para],1) # 5*batchsize 256 origin

            ## coarse to fine 训练，统一 把 img_para（5*batchsize，128，h ，w） 插值调整 为 5*batchsize 128 8 6
