...
if __name__ == '__main__':
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        with torch.no_grad():
            ...
            cv2.imwrite("./邓-face_align_img_resize.jpg",face_align_img_resize.squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255)
            
            pre_frame = model(face_align_img_resize, ref_img_tensor, deepspeech_tensor)
            ###################################   ###################################
            ...
            cv2.imwrite("./邓-pre_frame_origin_size.jpg",pre_frame_origin_size.squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255)
            ###################################   ###################################
