...
# landmark 行数对不上的版本
def load_landmark_openface(csv_path,num_frames):
    '''
    ...
    x_list = []
    y_list = []
    for row_index,row in enumerate(data_all[1:num_frames+1]):
...
## landmark 和 视频帧数对得上的版本
def load_landmark_openface_origin(csv_path):
    '''
    load openface landmark from .csv file
    '''
    with open(csv_path, 'r') as f:
...
def compute_crop_radius(video_size,landmark_data_clip,random_scale = None):
    ...
    radius_max = (np.int64(radius_max/4) + 1 ) * 4
