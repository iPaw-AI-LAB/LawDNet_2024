...
if __name__ == '__main__':
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        # crop_frame_data = cv2.resize(crop_frame_data, (resize_w,resize_h))  # [32:224, 32:224, :] # origin
        ...
        # crop_landmarks = torch.zeros( crop_frame_tensor.shape[0],68,2,device = device )
        # 检测landmark 并转为tensor导入
        # 检测单帧的landmark
        crop_landmarks_numpy, bboxes = torchlm.runtime.forward(crop_frame_data*255.0)
        crop_landmarks_tensor = torch.from_numpy(crop_landmarks_numpy).float().cuda()

        # 预处理增加遮罩
        crop_frame_tensor = smoothmask(crop_frame_tensor,crop_landmarks_tensor, resize_h, resize_w)
        # cv2.imwrite('./inference-实验2脸部_加mask之后.jpg', (crop_frame_tensor.squeeze().permute(1,2,0).cpu().numpy()*255)[:,:,::-1])
        ...
        with torch.no_grad():
            ...
            # cv2.imwrite('./inference-实验2脸部crop_生成结果.jpg', pre_frame[:, :, ::-1])
        ...
        # cv2.imwrite('./inference-实验2_全脸生成结果.jpg', frame_data[:, :, ::-1])
