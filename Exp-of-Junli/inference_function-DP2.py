...
def generate_video_with_audio(
    ...
    start_time_sec=0,
    max_frames=None,
    dp_2_model_path="model_path = 'D:\\LawDNet_2024\\Exp-of-Junli\\models_audio\\audio.ckpt",
):
    ...
    if (
        ...
        and gpu_index >= 0
        and torch.cuda.device_count() > gpu_index
    ):
    ...
    audio_model_path = dp_2_model_path
    precision = 16
    ...
    end_time = time.time()
    print(
        f"Running time: {end_time - start_time} seconds for extracting DeepSpeech features."
    )
    ...
    for frame in tqdm(
        video_frames, desc="Processing video frames, extracting landmarks..."
    ):
    ...
    reference_tensor = torch.tensor(
        video_frames[reference_index], dtype=torch.float
    ).to(device)
    reference_tensor = reference_tensor.permute(0, 3, 1, 2)
    reference_landmarks = torch.tensor(
        landmarks_list[reference_index], dtype=torch.float
    ).to(device)
    reference_tensor, _, _ = facealigner(
        reference_tensor, reference_landmarks, out_W=out_W
    )
    ...
    for i in tqdm(range(len_out // B), desc="Processing batches"):
        source_tensor = (
            torch.tensor(video_frames[i * B : (i + 1) * B].copy(), dtype=torch.float)
            .to(device)
            .permute(0, 3, 1, 2)
        )
        landmarks_tensor = torch.tensor(
            landmarks_list[i * B : (i + 1) * B], dtype=torch.float
        ).to(device)
        feed_tensor, _, affine_matrix = facealigner(
            source_tensor, landmarks_tensor, out_W=out_W
        )
        feed_tensor_masked = sqmasker(feed_tensor / 255.0)
        reference_tensor_B = (
            reference_tensor.unsqueeze(0)
            .expand(B, -1, -1, -1, -1)
            .reshape(B, 5 * 3, feed_tensor.shape[2], feed_tensor.shape[3])
        )
        audio_tensor = deepspeech_tensor[i * B : (i + 1) * B].to(device)

        start_time = time.time()
        output_B = (
            reference(net_g, feed_tensor_masked, reference_tensor_B, audio_tensor)
            .float()
            .clamp_(0, 1)
        )
        end_time = time.time()
        print(
            f"lawdnet 模型纯推理时间 {i} Reference tensor expanded in {end_time - start_time:.6f} seconds"
        )

        outframes_B = (
            facealigner.recover(output_B * 255.0, source_tensor, affine_matrix)
            .permute(0, 2, 3, 1)
            .cpu()
            .numpy()
            .astype(np.uint8)
    ...
    def save_video_with_audio(
        outframes, output_video_path, audio_path, output_dir, fps=25
    ):
        # 确保输出目录存在
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        ...
        command = [
            ...
            str(fps),  # 设置帧率
            "-i",
            os.path.join(temp_frames_path, "frame_%05d.png"),  # 输入图像序列
            "-c:v",
            "libx264",  # 编码器
            "-pix_fmt",
            "yuv420p",  # 像素格式
    ...
    output_video_path = os.path.join(
        output_dir, f"{timestamp_str}_{video_basename_without_ext}_{output_name}.mp4"
    )
    return save_video_with_audio(
        outframes, output_video_path, audio_path, output_dir, fps=25
    )
...
if __name__ == "__main__":
    ...
    video_path = "D:\\LawDNet_2024\\Exp-of-Junli\\models_video\\video.pth"

    audio_path = "D:\\LawDNet_2024\\Exp-of-Junli\\asset\women.wav"  #'./test_dp2_audio/taylor-20s.wav'
    output_dir = "./output_video"
    # 设置模型文件路径
    ...
    lawdnet_model_path = "D:\\LawDNet_2024\\Exp-of-Junli\\models_video\\video.pth"
    ...
    result_video_path = generate_video_with_audio(
        video_path,
        audio_path,
