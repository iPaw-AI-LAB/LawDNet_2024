...
from dataset.tensor_processing import SmoothMask
...
if __name__ == '__main__':
    ...
    # 显示进度条
    for frame_index in tqdm(range(num_frames),desc = "extracting landmark"):
        # print(frame_index,"帧")
        ret, frame = cap.read()
        if not ret:
            break
        ...
        # 将关键点结果保存到数组中
        video_landmark_data[frame_index] = landmarks[0]
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        ## 1.获取crop后的crop_landmark
        # import pdb
        # pdb.set_trace()
        landmarks, bboxs = torchlm.runtime.forward(crop_frame_data)
        crop_landmarks = (torch.from_numpy(landmarks))[0].unsqueeze(0)
        ...
        ############################################## smooth mask ##############################################
        crop_frame_tensor = torch.from_numpy(crop_frame_data).float().cuda().permute(2, 0, 1).unsqueeze(0) # origin
        ############################################## smooth mask ##############################################
        
        #  2. 用crop_landmark生成口罩mask
        crop_frame_tensor_mask = smoothmask(crop_frame_tensor, crop_landmarks, resize_h, resize_w)

        cv2.imwrite("./crop_frame_tensor_mask.jpg", crop_frame_tensor_mask[0].permute(1, 2, 0).cpu().numpy() * 255)
        ...
        with torch.no_grad():
            ...
            pre_frame = model(crop_frame_tensor_mask, ref_img_tensor, deepspeech_tensor)
