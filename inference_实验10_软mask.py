...
if __name__ == '__main__':
    ...
    model = LawDNet(opt.source_channel, opt.ref_channel, opt.audio_channel).cuda()
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        # face_align_img_resize[:,:,opt.mouth_region_size//2:opt.mouth_region_size//2 + opt.mouth_region_size,
        #                         opt.mouth_region_size//8:opt.mouth_region_size//8 + opt.mouth_region_size] = 0


        ################# 正方形的softmask  #################################
        face_align_img_resize_alpha = torch.zeros_like(face_align_img_resize[:,0:1,...])
        face_align_img_resize_alpha[:,:,opt.mouth_region_size//2:opt.mouth_region_size//2+opt.mouth_region_size,
                                opt.mouth_region_size//8:opt.mouth_region_size//8+opt.mouth_region_size] = 1
        face_align_img_resize_alpha = mask_gaussian(face_align_img_resize_alpha)
        face_align_img_resize_with_mask = face_align_img_resize*(1.0-face_align_img_resize_alpha)
        ################# 正方形的softmask  #################################
        ...
        with torch.no_grad():
            ...
            # pre_frame = model(crop_frame_tensor, ref_img_tensor, deepspeech_tensor) # origin
            cv2.imwrite("./邓-face_align_img_resize_with_mask.jpg",face_align_img_resize.squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255)
            
            pre_frame = model(face_align_img_resize_with_mask, ref_img_tensor, deepspeech_tensor)
            ###################################   ###################################
            pre_frame_origin_size = F.interpolate(pre_frame, size=(face_align_img.shape[2],face_align_img.shape[3]), mode='bilinear', align_corners=True)
            cv2.imwrite("./邓-face_align_img_resize_with_mask.jpg",pre_frame_origin_size.squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255)
            ###################################   ###################################
        ...
        reform_img = facealigner.recover((pre_frame_origin_size*255.0).float(), feed_img, face_align_matrix)
        reform_img_np = reform_img.permute(0,2,3,1).squeeze(0).cpu().numpy().astype(np.uint8) 
        # import pdb
        # pdb.set_trace()
        cv2.imwrite("./邓-reform_img_np.jpg",reform_img_np)
        videowriter.write(reform_img_np[:,:,::-1])
        ### 重写拼回去 逆变换正脸化  ####

        # videowriter.write(frame_data[:, :, ::-1]) # origin
    ...
    print("save path: {}".format(opt.res_video_dir))
